class GuitarFretboard extends HTMLElement {
    ...
    for (let s = 0; s < this.strings; s++) {
      for (let f = 0; f <= this.frets; f++) {
        ...
        if (this.tempFlashNotes.has(key)) {
          ...
          circle.setAttribute("fill", "rgba(255, 255, 255, 0.3)"); // Greyed-out tone
        }

        // Apply colors for chord tones
        if (this.isChordTone(key, 'tonic')) {
          circle.setAttribute("fill", "#ff8c00"); // Amber for tonic
        } else if (this.isChordTone(key, 'third')) {
          circle.setAttribute("fill", "#32cd32"); // Greenish for third
        } else if (this.isChordTone(key, 'fifth')) {
          circle.setAttribute("fill", "#4682b4"); // Bluish for fifth
        }
  ...
  // Helper function to identify chord tones
  isChordTone(key, type) {
    const [string, fret] = key.split("-").map(num => parseInt(num, 10));
    const note = this.scaleMap[string]?.[fret];
    if (!note) return false;

    // Define chord degrees (1st, 3rd, 5th)
    const scale = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
    const tonicIndex = scale.indexOf("G"); // Example: G Major (tonic is "G")
    const thirdIndex = (tonicIndex + 4) % 12;
    const fifthIndex = (tonicIndex + 7) % 12;

    // Tonic, Third, Fifth
    const degrees = {
      tonic: scale[tonicIndex],
      third: scale[thirdIndex],
      fifth: scale[fifthIndex],
    };

    return degrees[type] === note;
  }
