...
<title>Interactive Guitar + Piano Trainer</title>
<style>
  ...
  #note-panel .hint { color: #666; font-size: 12px; margin-bottom: 6px; }
...
class GuitarFretboard extends HTMLElement {
  constructor() {
    ...
    for (let s=1;s<=this.strings;s++){ this.activePerString.set(s,new Set()); this.wrongIndicators.set(s,new Set()); }

    this.tempFlashNotes = new Set();
    this.tempFlashPitches = new Set();

    // Sparse scale mapping for demonstration
    this.scaleMap = {
    ...
    this.pianoKeys = [
      {note:"C",type:"white"},{note:"C#",type:"black"},{note:"D",type:"white"},{note:"D#",type:"black"},
      {note:"E",type:"white"},{note:"F",type:"white"},{note:"F#",type:"black"},{note:"G",type:"white"},
      {note:"G#",type:"black"},{note:"A",type:"white"},{note:"A#",type:"black"},{note:"B",type:"white"}
    ];
  ...
  emitNoteChange() {
    const active = {};
    for(let s=1;s<=this.strings;s++){ active[s]=[...this.activePerString.get(s)].sort((a,b)=>a-b); }
    this.dispatchEvent(new CustomEvent("notechange",{detail:{activePerString:active,scaleMap:this.scaleMap,strings:this.strings},bubbles:true,composed:true}));
  }

  render() {
    const width=720,fretboardHeight=180,pianoHeight=140,verticalGap=24;
    ...
    for(let f=0;f<=this.frets;f++){
      ...
      const line=document.createElementNS(svgNS,"line");
      line.setAttribute("x1",x); line.setAttribute("y1",stringSpacing);
      line.setAttribute("x2",x); line.setAttribute("y2",fretboardHeight-stringSpacing);
      line.setAttribute("stroke",f===0?"black":"#888"); line.setAttribute("stroke-width",f===0?4:2);
      fretboard.appendChild(line);
    }
    ...
    for(let s=0;s<this.strings;s++){
      for(let f=0;f<=this.frets;f++){
        const key=`${s+1}-${f}`;
        const cx=fretSpacing*(f+0.5), cy=stringSpacing*(s+1);
        const circle=document.createElementNS(svgNS,"circle");
        circle.setAttribute("cx",cx); circle.setAttribute("cy",cy); circle.setAttribute("r",8);
        circle.setAttribute("stroke","black"); circle.style.cursor="pointer";
        const activeSet=this.activePerString.get(s+1), wrongSet=this.wrongIndicators.get(s+1);
        ...
        circle.addEventListener("click",()=>{
          if(!this.highlighted.has(key)){
            wrongSet.add(f); this.render();
            setTimeout(()=>{ wrongSet.delete(f); this.render(); this.emitNoteChange(); },2500); return;
          }
          const note=this.scaleMap[s+1]?.[f]; if(!note) return;
          activeSet.has(f)?activeSet.delete(f):activeSet.add(f);
          this.flashMatchingNotes(note); this.render(); this.emitNoteChange();
        });
    ...
    const spacer=document.createElement("div"); spacer.style.height=verticalGap+"px";
  ...
  renderPiano(svgNS,width,height){
    ...
    piano.setAttribute("width",width); piano.setAttribute("height",height);
    const whiteKeyWidth=width/7; let whiteIndex=0;

    this.pianoKeys.forEach(k=>{
      if(k.type!=="white") return;
      const x=whiteIndex*whiteKeyWidth;
      const rect=document.createElementNS(svgNS,"rect");
      rect.setAttribute("x",x); rect.setAttribute("y",0); rect.setAttribute("width",whiteKeyWidth); rect.setAttribute("height",height);
      rect.setAttribute("stroke","black"); rect.setAttribute("fill",this.tempFlashPitches.has(k.note)?"orange":"white");

      const label=document.createElementNS(svgNS,"text");
      label.setAttribute("x",x+whiteKeyWidth/2); label.setAttribute("y",height-12);
      label.setAttribute("text-anchor","middle"); label.setAttribute("font-size","14"); label.textContent=k.note;

      piano.appendChild(rect); piano.appendChild(label);
      whiteIndex++;
    });

    const blackKeyWidth=whiteKeyWidth*0.6; const blackKeyHeight=height*0.65;
    ...
    this.pianoKeys.forEach(k=>{
      ...
      const rect=document.createElementNS(svgNS,"rect");
      rect.setAttribute("x",x); rect.setAttribute("y",0); rect.setAttribute("width",blackKeyWidth); rect.setAttribute("height",blackKeyHeight);
      rect.setAttribute("fill",this.tempFlashPitches.has(k.note)?"orange":"black");

      const label=document.createElementNS(svgNS,"text");
      label.setAttribute("x",x+blackKeyWidth/2); label.setAttribute("y",blackKeyHeight-8);
      label.setAttribute("text-anchor","middle"); label.setAttribute("font-size","10");
      label.setAttribute("fill","white"); label.textContent=k.note;
  ...
  flashMatchingNotes(note){
    this.tempFlashNotes.clear(); this.tempFlashPitches.clear(); this.tempFlashPitches.add(note);
    ...
    this.render();
    setTimeout(()=>{ this.tempFlashNotes.clear(); this.tempFlashPitches.clear(); this.render(); },500);
  }
}

customElements.define("guitar-fretboard",GuitarFretboard);

// Page-level note panel update
